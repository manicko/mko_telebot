...
**Recommendation Stands:** Map Telethon exceptions to project exceptions, or catch Telethon hierarchy directly.
...
- **Architectural Risk:** The custom exception hierarchy (`TelegramAuthError`, `TelegramServiceError`) is well-designed but unused. Adoption requires systematic exception mapping throughout the Telethon integration layer.
- **Documentation Inconsistency:** `docs/SPEC.md` does not exist; README documents auth behavior but does not specify how errors are handled. The "graceful failure" implied by `start_client()` returning `False` cannot actually occur for real Telethon errors.
