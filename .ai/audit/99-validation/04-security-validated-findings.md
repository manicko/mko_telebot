...
|----------|---------|--------|
| MEDIUM | SEC-001 | In `cli.py:init()`, when `--force` is set, skip `telethon_config.yaml` if the user file exists (preserve credentials); only copy when absent. Add info log: "Preserved existing telethon_config.yaml". |
...
- **SEC-001**: Clarified to single code-based solution — preserve user credentials by excluding `telethon_config.yaml` from `--force` overwrite when user file exists, rather than documentation-only warning (the safer default).
