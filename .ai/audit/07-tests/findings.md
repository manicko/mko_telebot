...
**Evidence:** `src/mko_telebot/monitor_forward.py` lines 164-216 define `process_task`. No test file imports or tests this function. Search found 0 matches for `process_task` in tests.
...
**Evidence:** `src/mko_telebot/monitor.py` lines 117-133 define `run_monitor`. Search found 0 matches for `run_monitor` or `TestRun` in test files.
...
**Evidence:** `src/mko_telebot/monitor.py` lines 66-115 define `main_loop`. No test classes or functions reference this function.
...
**Evidence:** `src/mko_telebot/monitor.py` lines 20-37 define `reschedule_task`. Search found 0 matches in test files.
...
**Evidence:** `src/mko_telebot/logging.py` lines 22-46 define `setup_logging`. No test imports `setup_logging`.
...
**Evidence:** `tests/test_monitor.py` lines 466-536 show 7 tests using `patch("mko_telebot.monitor_forward.forward_to_users", ...)` and asserting mock call counts. Example at lines 466-468:
```python
...
**Evidence:** `tests/test_task.py` lines 275-298 test only success and generic ValueError. No tests cover rate limiting (FloodWaitError), network timeouts, or connection failures.
...
**Evidence:** `tests/test_task.py` lines 320-342. The test mocks `get_entity` side effects but doesn't verify the async sleep calls that would occur between entity resolutions.
...
**Evidence:** `tests/test_parser.py` only tests `search_match` via the `matcher` fixture. No tests import `PatternParser` or `parse_query` directly. No tests reference `ExactMatch`, `Wildcard`, `Exclusion`, `Sequence`, or `OrOperation` classes.
...
**Evidence:** `src/mko_telebot/core/config.py` lines 27-47 define `resolve_path`. No direct tests for this function.
