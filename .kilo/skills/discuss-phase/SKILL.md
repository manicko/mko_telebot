...
**DECISION_{file_number}.md feeds into:**

1. **researcher** — Reads DECISION_{file_number}.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **planner** — Reads DECISION_{file_number}.md to know WHAT decisions are locked
   - "Pull-to-refresh on mobile" → planner includes that in task specs
   - "KiloCode's Discretion: loading skeleton" → planner can decide approach
...
<philosophy>
**User = founder/visionary. KiloCode = builder.**
...
**KiloCode handles these (don't ask):**
...
Check if DECISION_{file_number}.md already exists:
...
If "View": Display DECISION_{file_number}.md, then offer update/skip
...
2. **Ask 4 questions using AskUserQuestion:**
   ...
   - Include "You decide" as an option when reasonable — captures KiloCode discretion
...
[Areas where user said "you decide" — note that KiloCode has flexibility here]
...
**Phase <next_available_number>: [Name]**

`/plan-phase.md <next_available_number>` — Plan the implementation for this phase, using the context we just created.
...
**Also available:**
- `/plan-phase.md <next_available_number> --skip-research` — plan without research
- Review/edit DECISION_<next_available_number>.md before continuing
...
- DECISION_{file_number}.md captures actual decisions, not vague vision
